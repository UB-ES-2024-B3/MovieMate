<template>
  <div class="flex flex-col h-screen">
    ...
    <div v-if="isMovies" class="flex flex-col sm:flex-row justify-center bg-gray-800 py-4 mb-6 rounded-lg shadow-md w-full">
      ...
      <!-- Filtro de Género -->
        <div class="filter-group">
          <label class="text-white font-semibold" for="genre">Género</label>
          <select id="genre" v-model="filters.genre" class="bg-gray-700 text-white px-4 py-2 rounded-md shadow-md">
        ...
        <div class="filter-group">
          <label class="text-white font-semibold" for="director">Director</label>
          <input
              id="director"
        ...
        <!-- Filtro de Actores -->
        <div class="filter-group">
          <label class="text-white font-semibold" for="actors">Actores</label>
          <input
              id="actors"
        ...
        <!-- Filtro de Año de Estreno (Más corto) -->
        <div class="filter-group short">
          <label class="text-white font-semibold" for="year">Año de Estreno</label>
          <input
              id="year"
        ...
        <!-- Filtro de Duración (Más corto) -->
        <div class="filter-group short">
          <label class="text-white font-semibold" for="duration">Duración (min)</label>
          <input
              id="duration"
        ...
        <!-- Filtro de Clasificación (Más corto) -->
        <div class="filter-group short">
          <label class="text-white font-semibold" for="classification">Clasificación</label>
        ...
        <!-- Filtro de Puntuación (Más corto) -->
        <div class="filter-group short">
          <label class="text-white font-semibold" for="score">Puntuación</label>
          <input
              id="score"
        ...
        <!-- Filtro de Total de Reseñas (Más corto) -->
        <div class="filter-group short">
          <label class="text-white font-semibold" for="reviews">Total de Reseñas</label>
          <input
              id="reviews"
    ...
    <div class="flex h-[calc(100vh-4rem)]">
      ...
      <main class="flex-1 bg-gray-900 flex items-start justify-center pt-8 sm:pt-6">
      <div
            v-if="showMessage"
            :class="[
        ...
        <section v-if="isMovies && movies.length > 0" class="relative w-full">
          ...
          <!-- Nueva Sección: Películas de Acción -->
          <h3 class="text-cyan-600 text-3xl font-bold mb-6 text-center">PELICULAS DE ACCIÓN</h3>
          <div class="relative overflow-hidden w-full">
            <!-- Flecha Izquierda -->
            <button
                class="absolute left-4 top-1/2 transform -translate-y-1/2 bg-gray-800 text-white rounded-full w-12 h-12 z-10 flex items-center justify-center shadow hover:bg-gray-700"
  ...
  methods: {
    ...
    async fetchFilterData() {
      try {
        ...
        // Fetch all filter data using axios
        const [genresResponse, directorsResponse, actorsResponse, classificationsResponse, premiereYearsResponse] = await Promise.all([
          axios.get(`${BASE_URL}/movie/filters/genres`),
          axios.get(`${BASE_URL}/movie/filters/directors`),
          axios.get(`${BASE_URL}/movie/filters/actors`),
          axios.get(`${BASE_URL}/movie/filters/classifications`),
          axios.get(`${BASE_URL}/movie/filters/premiere-years`)
        ]);

        // Assign the fetched data to the component's data properties
        this.genres = genresResponse.data;
        this.directors = directorsResponse.data;
        this.actors = actorsResponse.data;
        this.classifications = classificationsResponse.data;
        this.premiereYears = premiereYearsResponse.data;
    ...
    async applyFilters() {
      ...
      try {
        const payload = {
          genres: this.filters.genre || '', // Asignar vacío si no está seleccionado
          directors: this.filters.director || '',
          ...
          // Guarda las películas y la información de paginación en sessionStorage
          sessionStorage.setItem('filteredMovies', JSON.stringify({filters: payload }));

          // Redirige a la ruta 'FilteredMovies'
          this.$router.push({ name: 'FilteredMovies' });
...
/* Grupo de filtros */
.filter-group {
  flex: 1 1 calc(25% - 16px);  /* Ajusta el ancho de cada filtro */
  display: flex;
  flex-direction: column;
  min-width: 200px;            /* Evita que sean demasiado pequeños */
}

/* Filtros con ancho más corto */
.filter-group.short {
  flex: 1 1 calc(15% - 16px);  /* Ajusta el ancho de los filtros específicos */
...
@media (max-width: 1024px) {
  ...
  .filter-group {
    flex: 1 1 100%;  /* Cada filtro ocupa el ancho completo */
  }
}
