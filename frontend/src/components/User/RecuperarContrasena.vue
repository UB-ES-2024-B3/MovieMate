<template>
  <div class="flex h-[calc(100vh-4rem)] items-center justify-center">
    ...
    <!-- Contenedor de recuperar la contraseña-->
    <div class="w-full form_background_input rounded-lg shadow md:mt-0 sm:max-w-md xl:p-0">
      <div class="p-6 space-y-4 md:space-y-6 sm:p-8">
        <h2 class="text-xl font-bold text-center leading-tight tracking-tight form_title_text md:text-2xl">Recovery Password</h2>
        <form @submit.prevent="recovery">
          <!-- Input para nombre de usuario -->
          <div class="mb-4">
              <label for="password" class="block mb-2 text-sm font-medium form_title_text">New Password</label>
              <div class="relative">
                  <input
                          type="password"
                          name="password"
...
export default {
    ...
    methods: {
        ...
        async recovery() {
            ...
            // Validar que la fortaleza de la contraseña sea suficiente
            ...
            try {
                ...
                if (!token) {
                    this.showToastWithMessage('Authentication token is missing. Please try again.', true);
                    return;
                }
                ...
                if (response.status === 200) {
                    ...
                    this.showToastWithMessage(successMessage, false, () => {
                        this.$router.push('/login'); // Redirigir al login tras éxito
                    });
                }
            } catch (error) {
                // Manejar errores de la API
                if (error.response) {
                    // Extraer el código de estado y el mensaje de error
                    const { status, data } = error.response;
                    let errorMessage = 'Request failed. Please try again.';

                    // Evaluar según los códigos de estado especificados
                    if (status === 400) {
                    errorMessage = data.message || 'Invalid request.';
                    } else if (status === 404) {
                        errorMessage = data.message || 'The token is invalid or has expired.';
                    }

                    this.showToastWithMessage(errorMessage, true);
                } else {
                    // Manejar errores generales (sin respuesta)
                    console.error('Request error:', error);
                    this.showToastWithMessage('Unexpected error. Please try again.', true);
                }
            }
